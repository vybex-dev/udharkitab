...
export const REVENUECAT_API_KEY = "test_ROkpIIkloxOfKKFahuJqtTuUNEb";
...
export async function initRevenueCat(userId = null) {
  ...
  try {
    ...
    console.warn("⚠️ Failed to initialize RevenueCat (non-fatal):", error?.message || error);
  }
}
...
export async function getCustomerInfo() {
  ...
  try {
    ...
    if (isNetworkError(error)) {
      console.warn("RevenueCat getCustomerInfo note: network unavailable, using local cache.");
    } else {
...
export async function getOfferings() {
  ...
  try {
    ...
    if (isNetworkError(error)) {
      console.warn("RevenueCat getOfferings note: network unavailable (offline).");
    } else {
...
export async function purchasePackage(packageToBuy) {
  if (!isNativePurchasesAvailable || !isConfigured) {
    return {
      success: false,
      cancelled: false,
      error: "Native in-app purchases are not available in this build. Please run via development build ('npx expo run:android' or 'npx expo run:ios').",
    };
  }
  try {
...
export async function logInRevenueCat(userId) {
  ...
  try {
    ...
    if (isNetworkError(error)) {
      ...
      console.warn("RevenueCat login error (non-fatal):", error?.message || error);
    }
    return null;
  }
}
...
export async function logOutRevenueCat() {
  ...
  try {
    ...
    console.warn("RevenueCat logout error (non-fatal):", error?.message || error);
  }
}
    ...
    await RevenueCatUI.presentCustomerCenter({
      callbacks: {
        ...
        onFeedbackSurveyCompleted: ({ feedbackSurveyOptionId }) => {
          console.log("Customer feedback survey completed:", feedbackSurveyOptionId);
        },
      },
    });
  } catch (error) {
