...
async function pushTrialToCloud(localISO) {
  try {
    ...
    console.warn("pushTrialToCloud note (will retry when online):", e?.message || e);
    await setMeta(META_KEY_TRIAL_PENDING_SYNC, "1");
...
export async function getCombinedPlanStatus() {
  try {
    ...
    console.warn("getCombinedPlanStatus RevenueCat check note:", e?.message || e);
  }
...
export async function syncTrialWithCloud() {
  try {
    ...
    if (localValue) {
      ...
      return { synced: true, source: "local-pushed", trialStartedAt: localValue };
    }
