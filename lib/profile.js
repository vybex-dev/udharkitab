...
const META_KEY_LOCAL_DATA_OWNER_UID = "local_data_owner_uid";
...
export async function markOnboardingComplete() {
  ...
  // Stamp *who* this local SQLite data now belongs to. Local storage has no
  // per-account scoping of its own — it's just whatever khata was last
  // written on this device — so this is what lets _layout.jsx's AuthGate
  // notice a *different* account signing in later on a device that still
  // has someone else's data (e.g. from a sign-out that predates this
  // check), instead of silently showing it under the new account. See
  // getLocalDataOwnerUid() below.
  const uid = auth.currentUser?.uid;
  if (uid) await setMeta(META_KEY_LOCAL_DATA_OWNER_UID, uid);
}
...
/**
 * Which signed-in account's data is currently sitting in local SQLite, if
 * known. Stamped by markOnboardingComplete() above every time we're sure
 * local data belongs to whoever just signed in (fresh onboarding, or a
 * recognized returning-user recovery). Returns null if never stamped
 * (e.g. an install from before this check existed).
 */
export async function getLocalDataOwnerUid() {
  return getMeta(META_KEY_LOCAL_DATA_OWNER_UID);
}
...
export async function pushProfileToCloud({ shopName, theme, language, email, displayName } = {}) {
  try {
    ...
    console.warn("pushProfileToCloud note (will stay local-only for now):", e?.message || e);
 ...
 *
 * NOTE: shop name and theme are applied here because they live in the same
 * local store this module owns (SQLite app_meta, via setLocalShopName/
 * setLocalTheme). Language is deliberately NOT applied here — it lives in
 * AsyncStorage under LanguageContext's management (see LANGUAGE_KEY in
 * contexts/LanguageContext.js), not app_meta, and this module has no
 * business reaching into that store directly. Instead `row.language` is
 * returned so the caller — which already has useLanguage()'s setLanguage —
 * can apply it. That keeps AsyncStorage writes and the in-memory `language`
 * state changing together, instead of the two drifting out of sync (the
 * bug this replaced: nothing ever read row.language back, so returning
 * users landed back in the default language after a reinstall/new device).
 */
export async function pullProfileFromCloud() {
  try {
    ...
    if (row.trial_started_at) {
      await setMeta("trial_started_at", row.trial_started_at);
    }
    
    return { success: true, found: true, row, language: row.language || null };
  } catch (e) {
    console.warn("pullProfileFromCloud note (offline?):", e?.message || e);
    return { success: false, reason: "error", error: e };
  }
}
