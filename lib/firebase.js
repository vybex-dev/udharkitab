...
export async function signInWithGoogle() {
  try {
    if (isNativeAuthAvailable) {
      ...
      // Bridge into the JS SDK too — see the comment above jsAuth. Same
      // Google account + same Firebase project means the same uid on both
      // sides, so Firestore doc paths keyed by uid line up either way.
      // Best-effort: if this fails, native sign-in still succeeds and the
      // person stays logged in — they just won't get cloud sync until the
