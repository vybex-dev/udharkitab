 ...
 *
 * All functions reuse the same `getDB()`-style connection as db.js by
 * importing openDB indirectly via the shared SQLite instance — see the
 * `getDb()` helper below, which mirrors db.js's health-check pattern.
 */

import { getDB } from "./db";

// Reuses the exact same singleton connection as db.js (see getDB there).
// Opening a second independent connection to the same SQLite file on
// Android causes native-level crashes ("NullPointerException" from
// execAsync/prepareAsync), so this must never call SQLite.openDatabaseAsync
// directly.
