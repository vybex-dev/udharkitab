...
async function syncCustomerAfterWrite(customerId) {
  try {
    ...
    console.warn("syncCustomerAfterWrite: skipped (non-fatal):", e?.message || e);
  }
}
...
async function syncCustomerDeleteAfterWrite(customerId) {
  try {
    ...
    console.warn("syncCustomerDeleteAfterWrite: skipped (non-fatal):", e?.message || e);
  }
}
...
export async function enableCloudSync() {
  ...
  try {
    const db = await getDB();
    const customers = await db.getAllAsync(`SELECT id FROM customers;`);
    const bundles = (
    ...
    console.warn("enableCloudSync: initial push failed (non-fatal):", e?.message || e);
  }
  return { success: true };
}
...
export async function restoreCustomersFromCloud() {
  ...
  try {
        ...
        for (const payment of bundle.payments || []) {
          await db.runAsync(
            `INSERT OR REPLACE INTO payments (id, customer_id, amount, date, created_at)
             VALUES (?, ?, ?, ?, ?);`,
            [payment.id, bundle.id, payment.amount, payment.date, payment.created_at],
          );
    ...
    console.warn("restoreCustomersFromCloud: restore failed (non-fatal):", e?.message || e);
...
export async function settleEntry(entryId) {
  ...
  syncCustomerAfterWrite(entry.customer_id);
}
  ...
  try {
    ...
    if (overpayment > 0.009) {
      await db.runAsync(
        ...
        [customerId, overpayment, paymentId, date],
      );
    }
...
export async function deleteEntry(entryId) {
  ...
  if (entry) syncCustomerAfterWrite(entry.customer_id);
}
